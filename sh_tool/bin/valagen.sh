...
declare -A VALAGEN_LOGO_DATA=(
    [OWNER]="vroncevic"
    [REPO]="${VALAGEN_TOOL}"
    [VERSION]="${VALAGEN_VERSION}"
    [LOGO]="${VALAGEN_LOGO}"
)
...
#
# @brief   Main function 
# @params  PRoject name, project root dir, short description
...
function __valagen {
    ...
    if [ $NUMBER_OF_PARAMS -ne 4 ]; then
        usage VALAGEN_USAGE
        exit 128
    fi
    display_logo VALAGEN_LOGO_DATA
